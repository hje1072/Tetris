...
public class ScoreBoard {
	...
	public static String[] Mode = new String[11];
	...
	public void update() {
		...
		if(GamePanel.enteringScore) {
	    	...
	    	String name = JOptionPane.showInputDialog(null, "Please enter your name (max 8 characters):", "Enter your name", JOptionPane.QUESTION_MESSAGE);
	    	
	    	// 입력된 이름 출력
	    	if (name != null && !name.isEmpty()) {
	    		
	    		 this.name = name.substring(0, Math.min(name.length(), 8)); // 최대 6글자로 제한
	...
	public static void readscoreBoard() {
    	...
    	//파일을 읽어오는 과정.
        String csvFile = "src/data/scoreBoard.csv";
        String line = "";
        String csvSplitBy = ",";
	...
	public int writescoreBoard() {
        ...
        // 새로운 데이터
        String[] newData = {this.name, GamePanel.score, GamePanel.difficulty, "basic"};
        ...
        try (PrintWriter writer = new PrintWriter(new FileWriter(csvFile))) {
            // 헤더 쓰기
            writer.println("Name,Score,Difficulty,Mode");
	...
	public void draw(Graphics2D g2) {
		...
		g2.drawString(Difficulty[0], x + GamePanel.WIDTH * 1 / 4, y); //난이도
		g2.drawString(Mode[0], x + GamePanel.WIDTH * 2 / 5, y); //모드
		...
		for(int k = 1 ; k <= 10 ; k++) {
			...
			g2.drawString(Difficulty[k], x + GamePanel.WIDTH * 1 / 4, y); //난이도
			g2.drawString(Mode[k], x + GamePanel.WIDTH * 2 / 5, y); //스코어보드
